DPSMateFile("DPSMate_DataBuilder.lua")
-- NOtes
...
function DPSMate.DB:OnEvent(event)
	if event == "ADDON_LOADED" and (not self.loaded) then
		...
		-- Emberveil: laedt eine Modul-Datei nicht, brach diese Kette bisher ab und
		-- riss DPSMate.DB:OnEvent bei JEDEM Event mit. Fehlende Module ueberspringen.
		local moduleDB = {
			{ "DPS", DPSMateDamageDone },
			{ "Damage", DPSMateDamageDone },
			{ "DamageTaken", DPSMateDamageTaken },
			{ "DTPS", DPSMateDamageTaken },
			{ "EDD", DPSMateEDD },
			{ "EDT", DPSMateEDT },
			{ "FriendlyFire", DPSMateEDT },
			{ "FriendlyFireTaken", DPSMateEDT },
			{ "Healing", DPSMateTHealing },
			{ "HPS", DPSMateTHealing },
			{ "Overhealing", DPSMateOverhealing },
			{ "EffectiveHealing", DPSMateEHealing },
			{ "EffectiveHPS", DPSMateEHealing },
			{ "HealingTaken", DPSMateHealingTaken },
			{ "EffectiveHealingTaken", DPSMateEHealingTaken },
			{ "Absorbs", DPSMateAbsorbs },
			{ "AbsorbsTaken", DPSMateAbsorbs },
			{ "HealingAndAbsorbs", DPSMateAbsorbs },
			{ "Deaths", DPSMateDeaths },
			{ "Dispels", DPSMateDispels },
			{ "DispelsReceived", DPSMateDispels },
			{ "Decurses", DPSMateDispels },
			{ "DecursesReceived", DPSMateDispels },
			{ "CureDisease", DPSMateDispels },
			{ "CureDiseaseReceived", DPSMateDispels },
			{ "CurePoison", DPSMateDispels },
			{ "CurePoisonReceived", DPSMateDispels },
			{ "LiftMagic", DPSMateDispels },
			{ "LiftMagicReceived", DPSMateDispels },
			{ "Interrupts", DPSMateInterrupts },
			{ "AurasGained", DPSMateAurasGained },
			{ "AurasLost", DPSMateAurasGained },
			{ "AurasLost", DPSMateAurasGained },
			{ "AurasUptimers", DPSMateAurasGained },
			{ "Procs", DPSMateAurasGained },
			{ "Casts", DPSMateEDT },
			{ "Threat", DPSMateThreat },
			{ "TPS", DPSMateThreat },
			{ "Fails", DPSMateFails },
			{ "CCBreaker", DPSMateCCBreaker },
			{ "OHPS", DPSMateOverhealing },
			{ "OHealingTaken", DPSMateOverhealingTaken },
			{ "Activity", DPSMateCombatTime },
		}
		for i = 1, table.getn(moduleDB) do
			local mod = DPSMate.Modules[moduleDB[i][1]]
			if mod then mod.DB = moduleDB[i][2] end
		end
		...
		-- Faellt eine Datei aus, darf das nicht die ganze Initialisierung
		-- verhindern -- sonst werden die Fenster nie angelegt.
		if DPSMate.RegisterSelf then DPSMate:RegisterSelf() end
		DPSMate:OnLoad()
...
function DPSMate.DB:OnGroupUpdate()
	...
	for i=1, num do
		...
		if pet and pet ~= DPSMate.L["unknown"] then
			...
			DPSMate.Parser.TargetParty[swPet] = type.."pet"..i
		end
...
-- SuperWoW appends " (Owner)" to pets/traps/guardians in the combat log
-- e.g. "Kekris (Zooc)", "Explosive Trap III (Zooc)". DPSMate's merge only
-- knows UnitName("pet") ("Kekris"), so pet damage never joins the owner.
-- Returns: plainSource, ownerName  or  nil
function DPSMate.DB:ParseSuperWoWName(name)
	if not name or not strfind(name, "%(") then return end
	local _, _, source, owner = strfind(name, "^(.+) %(([^%)]+)%)$")
	if not source or not owner or owner == "" then return end
	-- SuperWoW special suffixes that are NOT owner names
	if owner == "self damage" or owner == "pet" or owner == "1" then return end
	-- Only treat as SuperWoW-owned if the parenthetical part is a real player
	if owner == UnitName("player") then
		return source, owner
	end
	if DPSMate.Parser.TargetParty and DPSMate.Parser.TargetParty[owner] then
		return source, owner
	end
	-- Known non-pet user (has class or was created as a player)
	if DPSMateUser[owner] and not DPSMateUser[owner][4] then
		return source, owner
	end
	return
end

-- Link a SuperWoW combat-log name (or plain pet) to its owner for merge/display
function DPSMate.DB:LinkPetOwner(petName, ownerName)
	if not petName or not ownerName or petName == ownerName then return end
	if self:BuildUser(ownerName, nil) then return end
	if self:BuildUser(petName, nil) then return end
	DPSMateUser[petName][4] = true
	DPSMateUser[petName][6] = DPSMateUser[ownerName][1]
	-- Prefer SuperWoW combat-log name as the merge target when present
	DPSMateUser[ownerName][5] = petName
end

-- Resolve combat-log source names for SuperWoW. Returns the name under which
-- damage/healing should be stored (owner if mergepets, otherwise original).
function DPSMate.DB:ResolveCombatLogSource(name)
	local source, owner = self:ParseSuperWoWName(name)
	if not source or not owner then
		return name
	end
	-- Always register the SuperWoW entity as a pet of the owner
	self:LinkPetOwner(name, owner)
	-- Also mark the plain pet unit name if it exists
	if source ~= name then
		self:BuildUser(source, nil)
		if DPSMateUser[source] then
			DPSMateUser[source][4] = true
			DPSMateUser[source][6] = DPSMateUser[owner][1]
		end
	end
	-- With merge enabled, attribute damage directly to the owner so pets,
	-- traps and temporary summons all count (SuperWoW can emit many names).
...
function DPSMate.DB:DamageDone(Duser, Dname, Dhit, Dcrit, Dmiss, Dparry, Ddodge, Dresist, Damount, Dglance, Dblock)
	Duser = self:ResolveCombatLogSource(Duser)
...
function DPSMate.DB:EnemyDamage(mode, arr, Duser, Dname, Dhit, Dcrit, Dmiss, Dparry, Ddodge, Dresist, Damount, cause, Dblock, Dcrush)
	-- SuperWoW: resolve pet/trap source names so EDD/EDT match DamageDone
	Duser = self:ResolveCombatLogSource(Duser)
	if self:BuildUser(Duser, nil) or self:BuildUser(cause, nil) or self:BuildAbility(Dname, nil) then return end
...
function DPSMate.DB:CombatTime()
	if not CombatFrame then
		...
		f:SetScript("OnUpdate", function(frame, elapsed)
			-- Wiki: SetScript ruft handler(self, ...) auf. OnUpdate liefert
			-- die Framezeit als zweites Argument, nicht als globales arg1.
			...
			if elapsed > 0.25 then elapsed = 0.05 end
			if (CombatState) then
				...
				if NextTotemDispel then
					TotemDispelTimer = TotemDispelTimer + elapsed
				...
				DPSMate.DB.MainUpdate = DPSMate.DB.MainUpdate + elapsed
			end
			if DPSMate.DB.NeedUpdate then
				MainLastUpdate = MainLastUpdate + elapsed
				if MainLastUpdate>=MainUpdateTime then
					DPSMate.DB:UpdateKicks()
					DPSMate.DB:UpdateThreat()
					pcall(function() DPSMate:SetStatusBarValue() end)
					DPSMate.DB.NeedUpdate = false
					MainLastUpdate = 0
				end
			end
...
-- Gleiche Absicherung: RepopMe ist eine Client-Funktion.
if not pcall(function() RepopMe = NewRepopMe end) then
	DPSMate.DB.RepopHookFailed = true
end
...
-- Lua-Eventframe, falls DPSMate_Options (letztes XML-Frame) nicht existiert
-- oder ADDON_LOADED dort nicht ankommt. Ohne diesen Pfad werden die Meter
-- erst beim Minimap-Klick gebaut.
do
	local boot = CreateFrame("Frame", "DPSMateBootFrame", UIParent)
	boot:RegisterEvent("ADDON_LOADED")
	boot:SetScript("OnEvent", function(self, ev, addon)
		ev = ev or event
		addon = addon or arg1
		if ev ~= "ADDON_LOADED" then return end
		if addon and addon ~= "DPSMate" then return end

...
local function ReleaseButton(self, level, index)
	...
	-- Emberveil: Widgets sind hier userdata, type() nennt sie nur "table".
	-- pairs() scheitert darauf. Statt alle Felder aufzuzaehlen werden genau die
	-- zurueckgesetzt, die Dewdrop selbst setzt (die Widget-Kinder text/check/
	-- arrow/colorSwatch/highlight/radioHighlight bleiben erhalten).
...
local function AcquireButton(self, level)
	...
	if table.getn(buttons) == 0 then
			...
			if not this.disabled then
				...
				elseif this.func then
					...
					if this.closeWhenClicked then
						self:Close()
					elseif level and level.IsShown and level:IsShown() then
		...
		button:SetScript("OnMouseDown", function(frame)
			if frame then this = frame end
			if this and not this.disabled and (this.func or this.colorFunc or this.closeWhenClicked) then
...
function Dewdrop:AddLine(k1, v1, k2, v2, k3, v3, k4, v4, k5, v5, k6, v6, k7, v7, k8, v8, k9, v9, k10, v10, k11, v11, k12, v12, k13, v13, k14, v14, k15, v15, k16, v16, k17, v17, k18, v18, k19, v19, k20, v20)
	...
	if info.textR and info.textG and info.textB then
		...
		-- Emberveil: GetTextColor gibt es nur auf FontString, nicht auf Font
		-- (https://emberveil.org/wiki/lua/widgets/Font). GetFontObject() liefert
		-- ein Font-Objekt, der Aufruf darauf schlaegt fehl. Farbe vom FontString
		-- selbst nehmen, sonst Standard-Gelb.
		local fo = button.text.GetFontObject and button.text:GetFontObject()
		if fo and fo.GetTextColor then
			button.text:SetTextColor(fo:GetTextColor())
		elseif button.text.GetTextColor then
			button.text:SetTextColor(button.text:GetTextColor())
		else
...
local function activate(self, oldLib, oldDeactivate)
	...
	-- Emberveil: everything below is convenience only -- close the open menu
	-- when the player clicks the world or when a FrameXML dropdown opens.
	-- Widgets are userdata on this client, so swapping WorldFrame's scripts or
	-- installing a method field on DropDownList1 can fail, and DropDownList1
	-- may not exist at all. An error here propagates out of activate() into
	-- AceLibrary:Register, which then never records the library -- so every
	-- later AceLibrary("DPSDewdrop-2.0") errors and takes DPSMate_Options.lua
	-- down with it. Losing click-to-close is survivable; that is not.
	local function installCloseHooks()
